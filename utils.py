...
async def load_events_df(
    ...
    Otherwise:
      - query both tables
      - combine them
      - save to multiple parquet chunks (to keep each file < ~50MB)
    ...
    if os.path.isdir(cache_dir):
        ...
        if files:
            ...
            # ensure we return with id as int index
    ...
    # 5) Cache to multiple parquet chunks to stay safely below 50MB/file
    ...
    # make sure id is stored as a clean string to keep fastparquet happy
    to_save["id"] = to_save["id"].astype("string")  # or .astype(str)
