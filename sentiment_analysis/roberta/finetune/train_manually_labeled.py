...
def prepare_datasets(config):
    ...
    pm_df.dropna(subset=['sentiment'], inplace=True)
    pm_df['text'] = pm_df['event_title'] + ' ' + pm_df['body']
    pm_df['label'] = pm_df['sentiment'].map({'Negative': 0, 'Positive': 1, 'Neutral': 2})
    ...
    test_dataset = Dataset.from_pandas(val_df[['text', 'label']])
...
def main():
    ...
    setproctitle.setproctitle("roberta_finetuning_worker")

    print(f"Original PID: {os.getpid()}")
    print("Process name changed to 'roberta_finetuning_worker' for easier identification.")
    ...
    if test_mode:
        ...
        tokenized_train = tokenized_train.shuffle(seed=42).select(range(min(100, len(tokenized_train))))
        tokenized_test = tokenized_test.shuffle(seed=42).select(range(min(20, len(tokenized_test))))
